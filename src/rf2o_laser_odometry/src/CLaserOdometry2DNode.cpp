...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode"), rf2o_ref(this->get_name())
{
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    if (rf2o_ref.first_laser_scan == false)
    {
      ...
      // Filter out INF and NaN values - replace with 0.0
      for (unsigned int i = 0; i < rf2o_ref.width; i++)
      {
        if (std::isfinite(new_scan->ranges[i]) && new_scan->ranges[i] > 0.0f)
          rf2o_ref.range_wf(i) = new_scan->ranges[i];
        else
          rf2o_ref.range_wf(i) = 0.0f;
      }
...
void CLaserOdometry2DNode::process()
{
  ...
  {
    // Process odometry estimation. Only publish when a new pose was actually
    // computed. If odometryCalculation() returns false the pose/timestamp were
    // not updated, and re-publishing would emit a duplicate, stale TF/odom that
    // downstream consumers (RViz, nav2) reject ("timestamp earlier than cache").
  ...
  else if (!rf2o_ref.is_initialized())
  {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000, "Waiting for laser_scans....");
  }
}
