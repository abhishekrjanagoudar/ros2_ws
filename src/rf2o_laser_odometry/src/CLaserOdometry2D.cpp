...
*/
CLaserOdometry2D::CLaserOdometry2D(const std::string& node_name) :
  node_name_(node_name),
  verbose(false),
  module_initialized(false),
  first_laser_scan(true),
  last_aver_residual_(0.0f),
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::msg::LaserScan& scan)
{
  ...
  auto m_runtime = rclcpp::Clock().now() - start;
...
void CLaserOdometry2D::findNullPoints()
{
  ...
  for (unsigned int u = 1; u < cols_i-1; u++)
  {
    ...
    {
      const float diff = std::abs(range_old[image_level](u) - range_warped[image_level](u));
      const float diff_max = std::min(0.2f, 0.05f*range_old[image_level](u));
...
void CLaserOdometry2D::solveSystemNonLinear()
{
  ...
  // ============================================================================
  // RESIDUAL CHECK: Store final average residual for quality assessment
  // ============================================================================
  // Recompute final average residual after IRLS
  aver_res = 0.f;
  for (unsigned int j=0; j < res.rows(); j++)
    aver_res += std::abs(res(j));
  aver_res /= res.rows();
  
  // Log for debugging (will use in filterLevelSolution)
  last_aver_residual_ = aver_res;
...
bool CLaserOdometry2D::filterLevelSolution()
{
  ...
  // NOTE: No covariance-magnitude rejection here (this matches upstream RF2O).
  // A previous version rejected solutions when cov_odo's smallest eigenvalue fell
  // below a fixed 1e-6 floor (reported as the cond=1e6 sentinel). That check was
  // structurally broken: cov_odo = (1/(n-3))*AtA^-1*||res||^2 shrinks with the
  // valid-point count n, so the finest, most reliable pyramid level almost always
  // trips the floor and gets discarded. Because a rejection here aborts the whole
  // scan (return false in the ctf loop), that froze the estimator entirely.
  // A small covariance eigenvalue means LOW uncertainty (good), not degeneracy,
  // so the gate was also backwards. The eigen-decomposition below is still used
  // for the complementary speed filter, exactly as in the original algorithm.
